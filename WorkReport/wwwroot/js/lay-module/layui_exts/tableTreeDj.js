...
layui.define(['table', 'jquery'], function (exports) {
    ...
    class Tree {
        ...
        _titleClick(e) {
            ...
            const key = that.getKeyId()

            // 兼容点到三角号的时候.
            let id=0
            if($(e.target).hasClass('layui-table-cell')) {
                id = $(e.target).parent().siblings('[data-field="'+ key +'"]').text()
            } else {
                id = $(e.target).parent().parent().siblings('[data-field="'+ key +'"]').text()
            }
            // 判断当前折叠还是展开
            const unfoldId = that.getUnfoldStatus(id);
            if(unfoldId) {
