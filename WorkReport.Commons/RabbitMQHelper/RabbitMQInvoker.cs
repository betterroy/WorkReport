...
namespace WorkReport.Commons.RabbitMQHelper
{
    ...
    public class RabbitMQInvoker
    {
        ...
        public RabbitMQInvoker(string hostName="127.0.0.1", string userName = "guest", string password = "guest")
        {
        ...
        public void SendDelay(string targetExchangeName, string message, int delaySecond)
        {
            ...
            using (var channel = _CurrentConnection.CreateModel())//开辟新的信道通信
            {
                try
                {
                    ...
                    ////DLX--- //死信队列绑定
                    //channel.ExchangeDeclare("MSA_exchange_dlx", "fanout", true, false, null);
                    //channel.QueueDeclare("MSA_queue_dlx", true, false, false, null);
                    //channel.QueueBind("MSA_queue_dlx", "MSA_exchange_dlx", "routingkey", null);
